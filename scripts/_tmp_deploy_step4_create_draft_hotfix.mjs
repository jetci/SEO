...
const HOTFIX_FILES = [
  { rel: 'server/routers/write.ts', desc: 'Backend createDraft zod refine 3-way + FRESH START GUARD (keywordId/draftId/raw keyword)' },
  { rel: 'client/src/pages/WritePage.tsx', desc: 'Frontend WritePage (payload raw keyword already pass; byte match confirm deployed)' },
];
...
async function main() {
  ...
  try {
    ...
    console.log('[4/10] Running Vite client build on remote (WritePage source byte match + write.ts backend)...');
    const buildStart = Date.now();
    const buildOut = await execLine(ssh, `cd ${REMOTE_ROOT} && timeout 240 npm run build 2>&1 | tail -25`);
    ...
    console.log('\n🏁 HOTFIX STEP4 CREATE DRAFT DEPLOY DONE — Summary:');
    console.log(`   • write.ts (Backend): createDraft accepts keywordId | draftId | raw keyword → FRESH START GUARD auto lookup/insert keyword row`);
    console.log(`   • WritePage.tsx (Frontend): doRunCreateDraft payload already sends keyword/category/intent/contentType → byte match confirmed`);
