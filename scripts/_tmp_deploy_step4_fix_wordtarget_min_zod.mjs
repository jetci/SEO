...
const HOTFIX_FILES = [
  { rel: 'server/routers/write.ts', desc: 'Backend createDraft outlineSections word_target_min min(60) → default(0) (allow 0 for H1 title row; Step3 had word_target_min=0 for H1 causing zod too_small error)' },
];
...
async function main() {
  ...
  try {
    ...
    // ===== UPLOAD + BYTE MATCH =====
    console.log(`[3/7] Upload ${HOTFIX_FILES.length} files (BE-only, no FE build needed)`);
    let ok = 0;
    ...
    console.log(`[7/7] GUARD V1 pid=1287 ACTUAL=${pid0After} ${guardAfter?'UNTOUCHED ✅':'FATAL ❗'} | V2=${v2Name||'?'} status=${v2Status} | Health :3002 phase=2:${p2?'OK':'NO'} ok:true=${rOK?'OK':'NO'} routers=${countRouters}/11 → ${p2&&rOK&&countRouters===11?'FULL HEALTH ✅':'DEGRADED ⚠️'}`);
    if (health.length > 0) console.log(`         RAW: ${health.slice(0,280)}`);
    const errTail = await execLine(ssh, 'tail -8 /home/ubuntu/.pm2/logs/eeat-studio-v2-error.log 2>/dev/null | grep -v "node --loader" | grep -vi "deprecation" | grep -v "SCHED PUB" | tail -5');
